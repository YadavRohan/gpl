   ...
   else if(m_kinds == EXPRESSION)
   {
      ...
      if(m_rhs) //binary operator
      {
         ...
         switch(m_oper)
         {
            ...
            case TOUCHES:
            {
                Game_object* lval = m_lhs->get_variable()->get_game_object_value();
                Game_object* rval = m_rhs->get_variable()->get_game_object_value();
                val = lval->touches(rval);
                break;
            }
            case NEAR:
            {
...
{
    //TODO chk if this is correct
    Animation_block* obj = NULL;
    if(m_kinds == VARIABLE)
    {
       if(m_var->get_type() != ANIMATION_BLOCK)
       {
           
       } 
       else
       {
           obj = m_var->get_animation_block_value();
       }
    }
    return obj;
}
