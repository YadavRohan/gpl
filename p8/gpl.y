...
#include <string>
    ...
    vector<Animation_block *> animation_vect;
    %} 
...
%type <union_statement_block> if_block statement_block end_of_statement_block
...
%type <union_int> check_animation_parameter
...
{
    //check animation vector to check if body for forward animation is provided
    for(auto it : animation_vect)
    {
...
simple_type  T_ID  optional_initializer
{
    ...
    if($1 == INT)
    {
        ...
        if($3)
        {
            if($3->get_type() != INT && $3->get_type() != INT_ARRAY && $3->get_type() != GAME_OBJECT)
            {
                //throw an error
...
T_ASSIGN expression
{
    //check if its content of the array
    Expression *expr = $2;
    $$ = $2;
}
| empty
...
T_LPAREN parameter_list_or_empty T_RPAREN
{
    ...
    Symbol *sym = new Symbol(*$2, cur_object_under_construction, 0,(Gpl_type)$1);
    stb->insert(*$2, sym);
}
...
{
     Event_manager::instance()->set_statement(Window::INITIALIZE, $2);
}
;

//---------------------------------------------------------------------
animation_block:
T_ANIMATION T_ID T_LPAREN check_animation_parameter T_ID
{
    Symbol_table *stb = Symbol_table::instance();
    Symbol *sym = stb->lookup(*$2);
    if(sym == NULL)
    {
    ...
    {
        if(sym->get_animation_block_value()->is_complete())
        {
            Error::error(Error::PREVIOUSLY_DEFINED_ANIMATION_BLOCK, sym->getName());
        }
        else
        {
            Symbol *ani_sym = sym->get_animation_block_value()->get_parameter_symbol();
            string type_name = ani_sym->get_game_object_value()->type();
            bool throw_error = false;
            if(ani_sym->getName() != *$5)
                throw_error = true;
            switch($4)
            {
                case T_TRIANGLE:
            ...
            statement_stack.push(sym->get_animation_block_value());
            sym->get_animation_block_value()->mark_complete();
        }
    }
} 
...
object_type T_ID
{
    ...
    stb->insert(*$2, new Symbol(*$2, cur_object_under_construction,0, (Gpl_type)$1));
}
;
...
{
    Event_manager::instance()->set_statement($2, $3);
}
;
...
T_IF T_LPAREN expression T_RPAREN if_block %prec IF_NO_ELSE
{
    ...
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new If_stmt(expr, $5) );
    }
    else
    {
...
| T_IF T_LPAREN expression T_RPAREN if_block T_ELSE if_block
{
    ...
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new If_stmt(expr, $5, $7) );
    }
    else
    {
        Error::error(Error::INVALID_TYPE_FOR_IF_STMT_EXPRESSION);
...
T_FOR T_LPAREN statement_block_creator assign_statement end_of_statement_block T_SEMIC expression T_SEMIC statement_block_creator assign_statement end_of_statement_block T_RPAREN statement_block
{
    ...
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new For_stmt(expr, $5, $11, $13) );
    }
    else
    {
...
T_PRINT T_LPAREN expression T_RPAREN
{
    ...
    if(type == INT || type == DOUBLE || type == STRING ||
       type == INT_ARRAY || type == DOUBLE_ARRAY || type == STRING_ARRAY
      )
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new Print_stmt(expr, $1));
    }
    else
    {
        //throw an error
...
T_EXIT T_LPAREN expression T_RPAREN
{
    ...
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new Exit_stmt(expr, $1));
    }
    else
    {
...
variable T_ASSIGN expression
{
    Variable *var = $1;
    Expression *expr = $3;
    Gpl_type vtype = var->get_type();
    Gpl_type etype = expr->get_type();
    if(etype > 1024)
        etype = (Gpl_type)(etype -1024);

    if(vtype < etype)
    {
        Error::error(Error::ASSIGNMENT_TYPE_ERROR, gpl_type_to_string(vtype), gpl_type_to_string(etype));
    } 
    else if (vtype == GAME_OBJECT) 
    {
        Error::error(Error::INVALID_LHS_OF_ASSIGNMENT, var->sym->getName(), gpl_type_to_string(var->sym->original_type));
    } 
    else if(vtype == ANIMATION_BLOCK)
    {
        Symbol *var_sym = var->get_symbol();
        Symbol *ani_sym = expr->eval_animation_block()->get_parameter_symbol();

        if(var_sym && ani_sym && var_sym->original_type != ani_sym->original_type)
        {
            Error::error(Error::ANIMATION_BLOCK_ASSIGNMENT_PARAMETER_TYPE_ERROR,
                        gpl_type_to_string(var_sym->original_type),
                        gpl_type_to_string(ani_sym->original_type)
                       );
        }
    }
    Statement_block *block = statement_stack.top();
    block->push_statement(new Assignment_stmt(expr, var, ASSIGN) );
}
| variable T_PLUS_ASSIGN expression
{
    ...
    if(vtype < etype)
    {
        Error::error(Error::PLUS_ASSIGNMENT_TYPE_ERROR, gpl_type_to_string(vtype), gpl_type_to_string(etype));
    } 
    else if (vtype == GAME_OBJECT) 
    {
        Error::error(Error::INVALID_LHS_OF_PLUS_ASSIGNMENT, var->sym->getName(), gpl_type_to_string(var->sym->original_type));
    } 
    Statement_block *block = statement_stack.top();
    block->push_statement(new Assignment_stmt(expr, var, PLUS_ASSIGN) );
}
| variable T_MINUS_ASSIGN expression
{
    ...
    if(vtype < etype)
    {
        Error::error(Error::MINUS_ASSIGNMENT_TYPE_ERROR, gpl_type_to_string(vtype), gpl_type_to_string(etype));
    } 
    else if (vtype == STRING || vtype == STRING_ARRAY) 
    {
        Error::error(Error::INVALID_LHS_OF_MINUS_ASSIGNMENT, var->get_member_var(), gpl_type_to_string(vtype));
    } 
    else
    {
        Statement_block *block = statement_stack.top();
        block->push_statement(new Assignment_stmt(expr, var, MINUS_ASSIGN) );
    }
}
;
...
| T_ID T_LBRACKET expression T_RBRACKET
{
    ...
    else
    {
        ...
        if(sym->get_type() != INT_ARRAY &&
                ...
                sym->get_type() != GAME_OBJECT_ARRAY
          )
        {
...
| variable geometric_operator variable
{
      ...
      if (ltype != GAME_OBJECT && ltype != GAME_OBJECT_ARRAY) 
      {
        Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, lhs->get_member_var());
 
        $$ = new Expression(0);
      } 
      if (rtype != GAME_OBJECT && rtype != GAME_OBJECT_ARRAY) 
      {
        Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, rhs->get_member_var());
      ...
      else 
      {
        ...
        Expression *l_expr = new Expression(lhs);
        Expression *r_expr = new Expression(rhs);
        $$ = new Expression(l_expr, $2, r_expr);
      }
}
;
